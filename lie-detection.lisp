...
(defparameter *dir* (translate-logical-pathname "ACT-R:models;"))

;; load file containing statistics functions
(smart-load *dir* "package.lisp")
(smart-load *dir* "lhstats.lisp")
...
(defun present-cue ()
  "Document here."
  (clear-exp-window)
  (add-text-to-exp-window :text (first *current-trial*) :x 125 :y 150)
  (setf *t1* (get-time))
...
(defun present-feedback (feedback)
  ...
  (clear-exp-window)
  (add-text-to-exp-window :text (first feedback) :x 125 :y 150)
  (proc-display :clear t))
...
(defun output-data-to-file (&optional (filename "data.csv"))
  ...
  (with-open-file (stream
                   (translate-logical-pathname
                    (format nil "ACT-R:models;~a" filename))
                   :direction :output
...
(defun runsim (nsubjects test-cond)
  "Run the experiment for n subjects for a particular condition (`easy' or
  `hard')."
  (let ((npercentages 7))
...
;; This is code for running an R script using SBCL on Linux that plots the model output
;;  (sb-ext:run-program
;;   "/usr/bin/Rscript"
;;  (list "/home/david/Dropbox/active/actr7/models/data-plots.R") :input t)))
